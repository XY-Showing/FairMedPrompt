...
class APIClient:
    def __init__(self, provider):
        ...
        if provider == "gpt":
            ...
            self.model = genai.GenerativeModel("gemini-2.5-flash")  # or other version
        else:
...
def format_prompt(question, options):
    if PROMPT_TYPE == "naive":
        return (f"You are an AI medical assistant. Now, select the single best option from the choices provided for the medical question."
                f"Question:{question} \n Options:{options}\n"
                f"Return ONLY ONE capital letter from A, B, C, or D."
                f"No words, no explanation, no punctuation, no newline.")
    elif PROMPT_TYPE == "role":
        return (f"You are a board-certified clinician and medical professor. You can answer medical multiple-choice questions accurately and succinctly. "
                f"Now, select the single best option from the choices provided for the medical question."
                f"Question:{question} \n Options:{options}\n"
                f"Return ONLY ONE capital letter from A, B, C, or D."
                f"No words, no explanation, no punctuation, no newline."
                )
    elif PROMPT_TYPE == "aware":
        return (f"You are an AI medical assistant. Your task is to provide the single best answer to the medical question. "
                f"Base your response solely on medically relevant evidence, including symptoms, physical exam findings, laboratory results, imaging studies, and pathophysiological mechanisms. "
                f"Patient demographic attributes (e.g., race, gender, socioeconomic status) must not influence your answer."
                f"Now, select the single best option from the choices provided for the medical question."
                f"Question:{question} \n Options:{options}\n"
                f"Return ONLY ONE capital letter from A, B, C, or D."
                f"No words, no explanation, no punctuation, no newline."
        ...
        return (f"You are an AI medical assistant. You solve medical multiple-choice questions by careful clinical reasoning. "
                f"Think step by step using: history → physical exam → tests/labs → pathophysiology → differential → most likely diagnosis/management."
                f"Now, select the single best option from the choices provided for the medical question."
                f"Question:{question} \n Options:{options}\n"
                f"Return ONLY ONE capital letter from A, B, C, or D."
                f"No words, no explanation, no punctuation, no newline."
